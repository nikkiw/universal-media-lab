...
# Remove previous toxics while keeping all proxies.
for p in dynamic clean wifi lte slow_lte three_g edge flaky offline; do
  for t in latency_up latency_down bandwidth_down packet_loss_down timeout_down; do
...
# Approximate presets. Latency is split between request and response paths; bandwidth is KB/s.
...
curl -fsS -X POST "$API/proxies/flaky/toxics" -H 'Content-Type: application/json' \
  -d '{"name":"packet_loss_down","type":"packet_loss","stream":"downstream","attributes":{"loss_rate":0.05,"correlation":0.30}}' >/dev/null
